...
Vous êtes maintenant prêt à déployer un modèle de langage d’IA générative pour prendre en charge votre application de conversation instantanée. Dans cet exemple, vous utiliserez le modèle OpenAI gpt-4o ; mais les principes restent les mêmes pour n’importe quel modèle.
...
1. Recherchez le commentaire **Obtenir une complétion de conversation**, puis modifiez le code afin d’ajouter l’entrée de l’utilisateur à l’invite, récupérer la complétion de votre modèle, et ajouter cette complétion à l’invite comme suit :
